...
-- Migrations intÃ©grÃ©es :
--   20260423_files_media_type_executable.sql
--   20260426_subscriptions_trial.sql
--   20260430_files_accessibility.sql
--   20260505_subscriptions_purchase_token_unique.sql
--   20260508_stripe_idempotency.sql
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `puzzle_devices` (
    `id`                 INT UNSIGNED  NOT NULL AUTO_INCREMENT,
    `device_uuid`        VARCHAR(100)  NOT NULL COMMENT 'UUID gÃ©nÃ©rÃ© cÃ´tÃ© app, unique, non modifiable',
    `device_token`       VARCHAR(64)   NOT NULL COMMENT 'Token opaque 64 chars (bin2hex 32 bytes)',
    `pseudonym`          VARCHAR(50)   NULL     COMMENT 'Choisi Ã  la premiÃ¨re utilisation du partage',
    `is_premium`         TINYINT(1)    NOT NULL DEFAULT 0,
    `purchase_token`     VARCHAR(500)  NULL     COMMENT 'Dernier token Google Play validÃ©',
...
-- -------------------------------------------------------
-- puzzle_shared
-- Casse-tÃªtes partagÃ©s entre deux appareils abonnÃ©s
-- -------------------------------------------------------
CREATE TABLE IF NOT EXISTS `puzzle_shared` (
    `id`               INT UNSIGNED   NOT NULL AUTO_INCREMENT,
    `shared_uid`       VARCHAR(36)    NOT NULL COMMENT 'UUID exposÃ© Ã  l\'app',
    `image_id`         INT UNSIGNED   NOT NULL,
    ...
    `completion`       TINYINT UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Pourcentage 0â€“100',
...
-- Ã‰tat courant de chaque piÃ¨ce (une ligne par piÃ¨ce, mise Ã  jour Ã  chaque move)
-- -------------------------------------------------------
...
-- Journal des mouvements pour le polling ; purgÃ© aprÃ¨s PUZZLE_EVENT_RETENTION_HOURS
-- -------------------------------------------------------
...
-- Subscriptions â€” v2.4.0
-- Migration 20260409 : table initiale
-- Migration 20260426 : device_token, stripe_customer, is_premium/show_ads/is_trial/trial_end,
--                      user_id nullable, contraintes hybrides uq_user_app / uq_device_app
...
CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    UNIQUE KEY uq_device_app        (device_token,  app_id),
    UNIQUE KEY uq_purchase_token_app (purchase_token, app_id),
