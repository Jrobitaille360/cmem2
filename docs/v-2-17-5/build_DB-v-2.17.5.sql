...
-- Version : 2.17.5 - 2026-09-02 (base historique 2.7.0)
-- ============================================================
...
--   v2.17.3 : 20260828_push_show_entity_detail.sql → notification_prefs.show_entity_detail
--   v2.17.5 : 20260901_stripe_subscriptions_is_premium.sql → stripe_subscriptions.is_premium
-- ============================================================
...
CREATE TABLE IF NOT EXISTS notification_prefs (
    ...
    show_entity_detail TINYINT(1) NOT NULL DEFAULT 0
...
-- ---------------------------------------------------------------------
-- Suivi du temps par tâche — sessions start/stop (D3)
-- Directive : 20260814_143000_cmem_web_vers_cmem2_API__time-tracking-sessions.md
-- Un seul minuteur actif à la fois par usager (contrainte posée en base via la
-- colonne générée active_user_id + index UNIQUE, pas seulement applicative).
-- `note` suit la convention e2e des VTODO : enc_alg/enc_iv nullable, opaque.
-- ---------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS `time_sessions` (
  ...
  `note` varchar(2000) DEFAULT NULL COMMENT 'Clair, ou base64 opaque si enc_alg renseigné',
  `enc_alg` varchar(32) DEFAULT NULL COMMENT 'Algorithme de chiffrement client, ex. AES-GCM-256. NULL = note en clair',
  `enc_iv` varchar(32) DEFAULT NULL COMMENT 'Vecteur d''initialisation base64. NULL = note en clair',
  `created_at` datetime DEFAULT current_timestamp(),
  `updated_at` datetime DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `active_user_id` int(11) GENERATED ALWAYS AS (if(`ended_at` is null,`user_id`,NULL)) STORED,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_time_sessions_active_user` (`active_user_id`),
  KEY `idx_time_sessions_todo` (`todo_id`),
  KEY `idx_time_sessions_user` (`user_id`),
