...
class DeviceTokenService
{
     ...
     */
    public static function generate(int $userId, string $deviceId, string $deviceName = 'Appareil inconnu'): string
    {
        ...
        $pdo->prepare(
            "INSERT INTO device_tokens
                (user_id, device_id, device_name, token_hash, expires_at, last_ip, last_ua)
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        )->execute([$userId, $deviceId, $deviceName, $hash, $expiresAt, $ip, $ua]);
    ...
    public static function validate(string $token, string $deviceId): ?array
    {
        ...
        if (!$record) {
            LogService::warning('Device token invalide ou expiré', [
                'device_id' => $deviceId,
            ]);
            return null;
        }
